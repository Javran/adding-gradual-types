...
a complex number. By the informal contract in the comment,
we know that input value is supposed to be either a number or
...
} \texttt{number?} distinguished these two different representations.
So in first branch of \texttt{cond}, we know for sure that \texttt{x} is a number,
whereas the second branch must deal with remaining case,
...
\begin{itemize}
	\item Difference occurrences of the same variable might have different types.
	\item A predicate distinguishes one particular type of value from others.
	For example, \texttt{number?} can distinguish numbers from other types.
	\item By reasoning about the execution path taken to reach certain point in the program,
	we can accumulate information about a variable to form a contextual type for it.
	For instance,
	in definition of \texttt{creal}, we assume \texttt{x} is either a number or a pair of numbers.
	By further evaluating \texttt{(number? x)}, each branch of \texttt{cond} expression
	have only one type of \texttt{x} to work with. 
\end{itemize}
