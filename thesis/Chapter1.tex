...
As we can see, when no dynamic type is involved, typechecking proceeds in the
exactly same way as that of a static type system.
To mimic dynamic type system, which does nothing ahead of execution,
we just assume that all terms have dynamic types.
With type consistency being a reflexive relation, no type error will be detected
during this phase.
...
So far we have seen that gradual type system attempts to accept
both statically and dynamically typed programs during static typechecking,
